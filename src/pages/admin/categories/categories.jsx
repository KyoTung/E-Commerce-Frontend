...
const Categories = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const title = currentCate.title.trim();
    if (!title) {
    ...
    const duplicate = categories.some(
      (c) =>
        ...
        (modalMode === "add" || c._id !== currentCate.id)
    );
    if (duplicate) {
    ...
    try {
      let resultAction;
      if (modalMode === "add") {
        resultAction = await dispatch(createCategory({ title }));
      } else {
            ...
            {filteredCategories.length === 0 ? (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {filteredCategories.map((cate, idx) => (
                    <tr key={cate._id} className="hover:bg-gray-50 transition">
                      <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-400">{idx + 1}</td>
                      <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{cate.title}</td>
            ...
            <form onSubmit={handleSubmit} className="p-5">
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder="VD: Gaming, Pin trâu,..."
                  autoFocus
                />
